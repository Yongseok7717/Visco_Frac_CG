...
def fTime1(tn):
    return 1.5*sqrt(tn)+1.0

def fTime2(tn):
    return math.gamma(2)/math.gamma(2.5)*(tn**1.5)+math.gamma(2.5)/2.0*(tn**2)
...
if k==1:
    np.savetxt("energy_error_linear.txt",V_error,fmt="%2.3e")
    np.savetxt("L2_error_linear.txt",L2_error,fmt="%2.3e")
elif k==2:
    np.savetxt("energy_error_quad.txt",V_error,fmt="%2.3e")
